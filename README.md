...
backend 容器启动时自动执行：`alembic upgrade head`（建 17 表）→ 种子（配置/认证用户/客户端/业务用户/数据源）→ 生成两个场景示例库数据（约 15 万行）→ 生成两组完整演示会话（可历史回放），**无需手工初始化**。
...
系统已预置两条完整分析链路，用 `analyst` 账号登录后直接在会话列表打开即可回放：

| 会话 | 数据源 | 链路内容 |
|---|---|---|
| 商品目录优化 · 6月信息流点击下滑归因 | 商品目录优化示例库 | 历史消息（问题→分析过程→结论）→ 六段式结果（指标4/证据3）→ 任务日志 → LLM 调用记录 |
| 库存异常分析 · SKU0001 华东仓断货归因 | 库存异常分析示例库 | 同上（指标3/证据3） |

适合演示：切换旧会话继续追问、结果区六部分结构化输出、任务/日志回看。重新生成：

```bash
python -m scripts.seed_demo_conversations            # 幂等重建（按会话标题）
python -m scripts.seed_demo_conversations --reset    # 先清空运行期数据再重建
python -m scripts.verify_demo_data                   # 接口级验证演示数据可回放
```
